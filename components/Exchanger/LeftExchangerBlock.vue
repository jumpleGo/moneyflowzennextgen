      ...
      <div class="exchanger__items">
        ...
        <div v-if="coinsForSell.length"  class="exchanger__items--list">
          <div v-for="(coin, index) in coinsForSell"
            ...
            <AppImage :image="`exchanger-valutes/${coin.key}.png`" :preload="{fetchPriority: 'high'}" width="25" height="25" />
            {{ coin.title }}
          </div>
        </div>
      </div>
      <div class="exchanger__items">
        ...
        <div v-if="valutesForSell.length" class="exchanger__items--list">
          <div v-for="(valute, index) in valutesForSell"
            ...
            <AppImage :image="`exchanger-valutes/${valute.key}.png`"  :preload="{fetchPriority: 'high'}" width="25" height="25" />
            {{ valute.title }}
          </div>
        </div>
      </div>
      ...
      <div class="exchanger__items">
        ...
        <div v-if="enabledOthers.length" class="exchanger__items--list">
          <div v-for="(coin, index) in enabledOthers"
            ...
            <AppImage :image="`exchanger-valutes/${coin.key}.png`" :preload="{fetchPriority: 'high'}" loading="eager" decoding="sync" />
            {{ coin.title }}
          </div>
        </div>
      </div>
